 ...
 * @file: file_from name
 * @fd_frm: file descriptor of file_from, or -1
 * @fd_to: file descriptor of file_to, or -1
 *
 * Return: void
 */
void check98(ssize_t check, char *file, int fd_frm, int fd_to)
{
	if (check == -1)
	{
 ...
 * Return: void
 */
void check99(ssize_t check, char *file, int fd_frm, int fd_to)
{
	if (check == -1)
	{
...
{
	int fd_frm, fd_to, close_to, close_frm;
	...
	check98((ssize_t)fd_frm, argv[1], -1, -1);
	file_perm = S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH;
	fd_to = open(argv[2], O_WRONLY | O_CREAT | O_TRUNC, file_perm);
	check99((ssize_t)fd_to, argv[2], fd_frm, -1);
	lengthr = 1024;
	while (lengthr == 1024)
	{
		lengthr = read(fd_frm, buffer, 1024);
		check98(lengthr, argv[1], fd_frm, fd_to);
		lengthw = write(fd_to, buffer, lengthr);
		if (lengthw != lengthr)
			lengthw = -1;
		check99(lengthw, argv[2], fd_frm, fd_to);
	}
	close_to = close(fd_to);
	close_frm = close(fd_frm);
	check100(close_to, fd_to);
	check100(close_frm, fd_frm);
	return (0);
}
